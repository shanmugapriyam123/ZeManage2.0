...
namespace BIManage.Addons.Views.Common
{
    ...
    public partial class ZeMessageBox : Window
    {
        ...
        {
            InitializeComponent();
            BIManage.Addons.Helpers.WindowRestoreHelper.PreventMinimize(this);
            LogoImage.Source = IconHelper.GetAppIconBitmap();
        }
        ...
        public static void Show(string title, string message, ZeMessageType type = ZeMessageType.Info, int autoCloseSeconds = 0)
        {
            ...
            // stacks above the dialog that triggered it (e.g. BulkExportMainWindow).
            // Without this, parenting to Revit's main HWND caused the success popup to
            // appear BEHIND the calling modal — the user couldn't see it.
            // Topmost is a safety net for cases where no active WPF window can be found
            // (e.g. background callers); it only applies for this dialog's lifetime.
